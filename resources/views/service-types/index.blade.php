        ...
        <div class="mx-auto px-4 sm:px-6 lg:px-8">
            <div class="mb-8">
                <div class="flex items-center justify-between">
                    <div>
                        <h1 class="text-3xl font-bold text-slate-900 dark:text-white">Service Types</h1>
                        <p class="mt-2 text-slate-600 dark:text-slate-400">Manage pre-authorization service types</p>
                    </div>
                    <a href="{{ route('service-types.create') }}" class="inline-flex items-center rounded-lg bg-emerald-600 px-4 py-2 font-medium text-white shadow-sm transition-colors duration-200 hover:bg-emerald-700">
                        ...
                        <tbody class="divide-y divide-slate-200 bg-white dark:divide-slate-700 dark:bg-slate-800">
                            @forelse($serviceTypes as $index => $serviceType)
                                <tr class="hover:bg-slate-50 dark:hover:bg-slate-700">
                                    ...
                                    <td class="whitespace-nowrap px-6 py-4 text-sm font-medium text-slate-900 dark:text-white">{{ $serviceType->name }}</td>
                                    <td class="whitespace-nowrap px-6 py-4 text-sm font-medium">
                                        <div class="flex gap-3">
                                            <a href="{{ route('service-types.edit', $serviceType) }}" class="text-blue-600 hover:text-blue-900 dark:text-blue-400 dark:hover:text-blue-300">
                                                <i class="fa-solid fa-edit mr-1"></i> Edit
                                            </a>
                                            <form action="{{ route('service-types.destroy', $serviceType) }}" method="POST" class="delete-form">
                                                @csrf
                                ...
                                <tr>
                                    <td class="px-6 py-12 text-center" colspan="3">
                                        <div class="text-slate-500 dark:text-slate-400">
                                            <i class="fa-solid fa-list mb-4 text-4xl"></i>
                                            <p class="text-lg font-medium">No service types found</p>
                                            <p class="mt-1">Add your first service type.</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
