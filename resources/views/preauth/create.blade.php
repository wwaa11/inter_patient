    ...
    <div class="min-h-screen bg-slate-50 py-8 dark:bg-slate-900">
        <div class="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
            ...
            <div class="rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-800">
                <form action="{{ route('preauth.store') }}" method="POST" enctype="multipart/form-data" class="p-8" id="preauth-form">
                    @csrf
                    <div class="space-y-6">
                        ...
                        <div class="grid gap-6 sm:grid-cols-2">
                            <div>
                                ...
                                <input type="text" name="hn" id="hn" value="{{ old('hn') }}" required
                                    class="mt-1 block w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-slate-900 shadow-sm focus:border-emerald-500 focus:outline-none focus:ring-emerald-500 dark:border-slate-600 dark:bg-slate-700 dark:text-white sm:text-sm"
                            ...
                            <div>
                                ...
                                <div class="relative mt-1">
                                    <input type="text" name="patient_name" id="patient_name" value="{{ old('patient_name') }}" readonly
                        ...
                        <div>
                            ...
                            <input type="date" name="date_of_service" id="date_of_service" value="{{ old('date_of_service') }}"
                                class="mt-1 block w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-slate-900 shadow-sm focus:border-emerald-500 focus:outline-none focus:ring-emerald-500 dark:border-slate-600 dark:bg-slate-700 dark:text-white sm:text-sm">
                        ...
                        <div class="grid gap-6 sm:grid-cols-2">
                            ...
                            <div>
                                ...
                                <input type="datetime-local" name="requested_date" id="requested_date" value="{{ old('requested_date') }}"
                                    class="mt-1 block w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-slate-900 shadow-sm focus:border-emerald-500 focus:outline-none focus:ring-emerald-500 dark:border-slate-600 dark:bg-slate-700 dark:text-white sm:text-sm">
                        ...
                        @if($adminUsers->isNotEmpty())
                            <div class="relative">
                                <label for="handling_staff_search" class="block text-sm font-medium text-slate-700 dark:text-slate-300">Handling Staffs</label>
                                <div class="searchable-dropdown mt-1 rounded-lg border border-slate-300 bg-white shadow-sm dark:border-slate-600 dark:bg-slate-800">
                        ...
                        <div>
                            ...
                            <select name="case_status" id="case_status" required class="mt-1 block w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-slate-900 shadow-sm focus:border-emerald-500 focus:outline-none focus:ring-emerald-500 dark:border-slate-600 dark:bg-slate-700 dark:text-white sm:text-sm">
                                @foreach(\App\Models\PreAuthorization::caseStatusOptions() as $opt)
                                    <option value="{{ $opt }}" {{ old('case_status', 'Data Entered') === $opt ? 'selected' : '' }}>{{ $opt }}</option>
                                @endforeach
                            </select>
                            @error('case_status')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        ...
                        <div>
                            ...
                            <select name="coverage_decision" id="coverage_decision" class="mt-1 block w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-slate-900 shadow-sm focus:border-emerald-500 focus:outline-none focus:ring-emerald-500 dark:border-slate-600 dark:bg-slate-700 dark:text-white sm:text-sm">
                                ...
                                @foreach(\App\Models\PreAuthorization::coverageDecisionOptions() as $opt)
                                    <option value="{{ $opt }}" {{ old('coverage_decision') === $opt ? 'selected' : '' }}>{{ $opt }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input type="date" name="send_out_date" id="send_out_date" value="{{ old('send_out_date') }}"
                                class="mt-1 block w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-slate-900 shadow-sm focus:border-emerald-500 focus:outline-none focus:ring-emerald-500 dark:border-slate-600 dark:bg-slate-700 dark:text-white sm:text-sm">
                            @error('send_out_date')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            <label class="block text-sm font-medium text-slate-700 dark:text-slate-300">GOP Attachments (PDF or image, multiple)</label>
                    ...
                    <div class="mt-8 flex justify-end gap-4">
                        <a href="{{ route('preauth.index') }}" class="inline-flex items-center rounded-lg border border-slate-300 bg-white px-4 py-2 font-medium text-slate-700 shadow-sm hover:bg-slate-50 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-300 dark:hover:bg-slate-600">Cancel</a>
                        <button type="submit" class="inline-flex items-center rounded-lg bg-emerald-600 px-6 py-2 font-medium text-white shadow-sm hover:bg-emerald-700 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2">Create</button>
                    </div>
                </form>
            </div>
        </div>
            ...
            (function() {
                ...
                function renderFileList() {
                    ...
                    for (var i = 0; i < gopFileList.length; i++) {
                        ...
                        var preview = '';
                        if (isImage(file)) {
                            preview = '<img src="' + URL.createObjectURL(file) + '" alt="" class="h-16 w-full object-cover rounded-t-lg">';
                        } else if (isPdf(file)) {
