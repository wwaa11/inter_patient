...
@section('content')
    <div class="min-h-screen bg-slate-50 py-8 dark:bg-slate-900">
        <div class="mx-auto w-full px-4 sm:px-6 lg:px-8">
            <div class="mb-8">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p class="mt-2 text-lg text-slate-600 dark:text-slate-400">HN {{ $preauth->hn }} —
                            {{ $preauth->patient_name ?? '—' }}</p>
                    </div>
                    @if (auth()->user()->isAdmin())
                        <div class="ml-auto flex flex-wrap items-center gap-3">
                            ...
                            <a href="{{ route('admissions.create', ['from_preauth' => 1, 'preauth_id' => $preauth->id]) }}"
                                class="inline-flex items-center rounded-lg bg-emerald-600 px-4 py-2 font-medium text-white shadow-sm hover:bg-emerald-700">
                                <i class="fa-solid fa-bed-pulse mr-2"></i>
                                Create Admission
                            </a>
                            <form action="{{ route('preauth.destroy', $preauth) }}" method="POST"
                                class="inline delete-preauth-form">
                                @csrf
                                <button type="button"
                                    class="delete-preauth-btn inline-flex items-center rounded-lg border border-red-200 bg-white px-4 py-2 font-medium text-red-700 shadow-sm hover:bg-red-50 dark:border-red-800 dark:bg-slate-700 dark:text-red-400 dark:hover:bg-red-900/20">
            ...
            <div id="view-mode-section">
                ...
                <div
                    ...
                    <dl class="grid grid-cols-1 gap-x-4 gap-y-8 sm:grid-cols-2 lg:grid-cols-3">
                        <div>
                            ...
                            <dd class="mt-1 text-slate-900 dark:text-white">{{ $preauth->hn }}</dd>
                        </div>
                        <div>
                            <dt class="text-xs font-medium uppercase text-slate-500 dark:text-slate-400">Patient Name</dt>
                            <dd class="mt-1 text-slate-900 dark:text-white">{{ $preauth->patient_name ?? '—' }}</dd>
                        </div>
                        <div>
                            <dt class="text-xs font-medium uppercase text-slate-500 dark:text-slate-400">Service Type</dt>
                            <dd class="mt-1 text-slate-900 dark:text-white">{{ $preauth->serviceType?->name ?? '—' }}</dd>
                        </div>
                        <div>
                            <dt class="text-xs font-medium uppercase text-slate-500 dark:text-slate-400">Date of Service
                            </dt>
                            <dd class="mt-1 text-slate-900 dark:text-white">
                                {{ $preauth->date_of_service?->format('Y-m-d') ?? '—' }}</dd>
                        </div>
                        <div>
                            <dt class="text-xs font-medium uppercase text-slate-500 dark:text-slate-400">Requested Date</dt>
                            <dd class="mt-1 text-slate-900 dark:text-white">
                                {{ $preauth->requested_date?->format('Y-m-d H:i') ?? '—' }}</dd>
                        </div>
                        <div>
                            ...
                            <dd class="mt-1">
                                @if ($preauth->send_out_date)
                                    <span
                                        class="text-slate-900 dark:text-white">{{ $preauth->send_out_date->format('Y-m-d') }}</span>
                                    <span
                                        class="ml-2 text-sm text-slate-500 dark:text-slate-400">({{ $preauth->send_out_date->diffForHumans() }})</span>
                                    @if ($preauth->send_out_date->isPast())
                                        <span
                                            class="ml-2 inline-flex rounded-full bg-red-100 px-2.5 py-0.5 text-xs font-medium text-red-800 dark:bg-red-900 dark:text-red-200">Overdue</span>
                                    @endif
                                @else
                                    —
                                @endif
                        ...
                        <div>
                            <dt class="text-xs font-medium uppercase text-slate-500 dark:text-slate-400">Notifier</dt>
                            <dd class="mt-1 text-slate-900 dark:text-white">{{ $preauth->notifier?->name ?? '—' }}</dd>
                        </div>
                        <div>
                            <dt class="text-xs font-medium uppercase text-slate-500 dark:text-slate-400">Handling Staffs
                            </dt>
                            <dd class="mt-1 text-slate-900 dark:text-white">
                                {{ $preauth->handlingStaffs->pluck('name')->join(', ') ?: '—' }}</dd>
                        </div>
                        <div>
                            <dt class="text-xs font-medium uppercase text-slate-500 dark:text-slate-400">Case Status</dt>
                            <dd class="mt-1"><span
                                    class="inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium bg-slate-100 text-slate-800 dark:bg-slate-700 dark:text-slate-200">{{ $preauth->case_status ?? '—' }}</span>
                            </dd>
                        </div>
                    </dl>
                </div>
                ...
                <div
                    class="mb-8 rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-800">
                    <h2 class="mb-6 text-xl font-semibold text-slate-900 dark:text-white">Provider</h2>
                    <div
                        class="mb-8 flex flex-col gap-4 sm:flex-row sm:items-start border-b border-slate-100 pb-6 dark:border-slate-700">
                        @if ($preauth->provider->logo)
                            <img src="{{ asset('provider-logos/' . $preauth->provider->logo) }}"
                                alt="{{ $preauth->provider->name }}"
                                class="h-20 w-auto shrink-0 object-contain rounded-lg border border-slate-200 dark:border-slate-600">
                        @else
                            <div
                                class="flex h-20 w-20 shrink-0 items-center justify-center rounded-lg border border-slate-200 bg-slate-100 dark:border-slate-600 dark:bg-slate-700">
                        ...
                        <div class="min-w-0 flex-1">
                            ...
                            <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Type: {{ $preauth->provider->type }}
                            </p>
                            @if ($preauth->provider->detail)
                                <div
                                    class="mt-3 rounded-lg bg-slate-50 p-3 text-sm text-slate-700 dark:bg-slate-900 dark:text-slate-300">
                    ...
                    <dl class="grid grid-cols-1 gap-x-4 gap-y-8 sm:grid-cols-2 lg:grid-cols-3">
                        <div class="sm:col-span-2 lg:col-span-3">
                            <dt class="text-xs font-medium uppercase text-slate-500 dark:text-slate-400">
                                Operations/Procedures</dt>
                            <dd class="mt-1 whitespace-pre-wrap text-slate-900 dark:text-white">
                                {{ $preauth->operations_procedures ?? '—' }}</dd>
                        </div>
                        <div>
                            <dt class="text-xs font-medium uppercase text-slate-500 dark:text-slate-400">Coverage Decision
                            </dt>
                            <dd class="mt-1 text-slate-900 dark:text-white">{{ $preauth->coverage_decision ?? '—' }}</dd>
                        </div>
                        <div>
                            <dt class="text-xs font-medium uppercase text-slate-500 dark:text-slate-400">GOP Reference
                                Number</dt>
                            <dd class="mt-1 text-slate-900 dark:text-white">{{ $preauth->gop_reference_number ?? '—' }}
                            </dd>
                        </div>
                        <div>
                            <dt class="text-xs font-medium uppercase text-slate-500 dark:text-slate-400">GOP Receiving Date
                            </dt>
                            <dd class="mt-1 text-slate-900 dark:text-white">
                                {{ $preauth->gop_receiving_date?->format('Y-m-d') ?? '—' }}</dd>
                        </div>
                        <div>
                            <dt class="text-xs font-medium uppercase text-slate-500 dark:text-slate-400">GOP Translate By
                            </dt>
                            <dd class="mt-1 text-slate-900 dark:text-white">
                                {{ $preauth->gopTranslateByUser?->name ?? '—' }}</dd>
                        </div>
                    </dl>
                </div>
            </div>
            ...
            @if (auth()->user()->isAdmin())
                <div id="edit-mode-section"
                    ...
                    <div
                        class="border-b border-slate-200 px-6 py-4 dark:border-slate-700 flex justify-between items-center">
                    ...
                    <form action="{{ route('preauth.update', $preauth) }}" method="POST" enctype="multipart/form-data"
                        class="p-6 space-y-8" id="preauth-form">
                        @csrf
                        ...
                        <div class="space-y-6">
                            ...
                            <div class="grid gap-6 sm:grid-cols-2">
                                <div>
                                    ...
                                    <input type="text" name="hn" id="hn"
                                        value="{{ old('hn', $preauth->hn) }}" required
                                ...
                                <div>
                                    ...
                                    <div class="relative mt-1">
                                        <input type="text" name="patient_name" id="patient_name"
                                            value="{{ old('patient_name', $preauth->patient_name) }}" readonly
                            ...
                            <div class="grid gap-6 sm:grid-cols-2">
                                <div>
                                    ...
                                    <select name="service_type_id" id="service_type_id" required
                                        ...
                                        @foreach ($serviceTypes as $st)
                                            <option value="{{ $st->id }}"
                                                {{ old('service_type_id', $preauth->service_type_id) == $st->id ? 'selected' : '' }}>
                                                {{ $st->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div>
                                    <label for="date_of_service"
                                        class="block text-sm font-medium text-slate-700 dark:text-slate-300">Date of
                                        Service</label>
                                    <input type="date" name="date_of_service" id="date_of_service"
                                        value="{{ old('date_of_service', $preauth->date_of_service?->format('Y-m-d')) }}"
                                        class="mt-1 block w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-slate-900 shadow-sm focus:border-emerald-500 focus:outline-none focus:ring-emerald-500 dark:border-slate-600 dark:bg-slate-700 dark:text-white sm:text-sm">
                                </div>
                            </div>
                            ...
                            <div class="grid gap-6 sm:grid-cols-2">
                                <div>
                                    ...
                                    <input type="datetime-local" name="requested_date" id="requested_date"
                                        value="{{ old('requested_date', $preauth->requested_date?->format('Y-m-d\TH:i')) }}"
                                        class="mt-1 block w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-slate-900 shadow-sm focus:border-emerald-500 focus:outline-none focus:ring-emerald-500 dark:border-slate-600 dark:bg-slate-700 dark:text-white sm:text-sm">
                                </div>
                                <div>
                                    <label for="send_out_date"
                                        class="block text-sm font-medium text-slate-700 dark:text-slate-300">Send Out
                                        Date</label>
                                    <input type="date" name="send_out_date" id="send_out_date"
                                        value="{{ old('send_out_date', $preauth->send_out_date?->format('Y-m-d')) }}"
                                        class="mt-1 block w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-slate-900 shadow-sm focus:border-emerald-500 focus:outline-none focus:ring-emerald-500 dark:border-slate-600 dark:bg-slate-700 dark:text-white sm:text-sm">
                                </div>
                            </div>
                            ...
                            <div class="grid gap-6 sm:grid-cols-2">
                                <div class="relative">
                                    ...
                                    <div id="notifier_selected" class="mt-2 flex flex-wrap gap-2">
                                        ...
                                        @else
                                            <input type="hidden" name="notifier_id" value=""
                                                id="notifier_empty_input">
                                        @endif
                                    </div>
                                ...
                                <div>
                                    ...
                                    <select name="case_status" id="case_status"
                                        ...
                                        @foreach (\App\Models\PreAuthorization::caseStatusOptions() as $opt)
                                            <option value="{{ $opt }}"
                                                {{ old('case_status', $preauth->case_status) === $opt ? 'selected' : '' }}>
                                                {{ $opt }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            ...
                            <div>
                                <label class="block text-sm font-medium text-slate-700 dark:text-slate-300">Handling
                                    Staffs</label>
                                <div
                                    class="searchable-dropdown mt-1 rounded-lg border border-slate-300 bg-white shadow-sm dark:border-slate-600 dark:bg-slate-800">
                        ...
                        <div class="space-y-6 pt-6 border-t border-slate-200 dark:border-slate-700">
                            ...
                            <div class="relative">
                                ...
                                <div
                                    class="searchable-dropdown mt-1 rounded-lg border border-slate-300 bg-white shadow-sm dark:border-slate-600 dark:bg-slate-800">
                                    <input type="text" id="payor_search" placeholder="Search and click to select..."
                                        autocomplete="off"
                                        class="block w-full rounded-t-lg border-0 bg-transparent px-3 py-2 text-slate-900 placeholder-slate-400 focus:ring-0 dark:text-white dark:placeholder-slate-500 sm:text-sm">
                            ...
                            <div class="grid gap-6 sm:grid-cols-2">
                                <div>
                                    ...
                                    <select name="coverage_decision" id="coverage_decision"
                                        ...
                                        @foreach (\App\Models\PreAuthorization::coverageDecisionOptions() as $opt)
                                            <option value="{{ $opt }}"
                                                {{ old('coverage_decision', $preauth->coverage_decision) === $opt ? 'selected' : '' }}>
                                                {{ $opt }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div>
                                    ...
                                    <input type="text" name="gop_reference_number" id="gop_reference_number"
                                        value="{{ old('gop_reference_number', $preauth->gop_reference_number) }}"
                                        class="mt-1 block w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-slate-900 shadow-sm focus:border-emerald-500 focus:outline-none focus:ring-emerald-500 dark:border-slate-600 dark:bg-slate-700 dark:text-white sm:text-sm">
                                </div>
                            </div>
                            ...
                            <div class="grid gap-6 sm:grid-cols-2">
                                <div>
                                    ...
                                    <input type="date" name="gop_receiving_date" id="gop_receiving_date"
                                        value="{{ old('gop_receiving_date', $preauth->gop_receiving_date?->format('Y-m-d')) }}"
                                        class="mt-1 block w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-slate-900 shadow-sm focus:border-emerald-500 focus:outline-none focus:ring-emerald-500 dark:border-slate-600 dark:bg-slate-700 dark:text-white sm:text-sm">
                                </div>
                                <div>
                                    ...
                                    <select name="gop_translate_by" id="gop_translate_by"
                                        ...
                                        @foreach ($adminUsers as $u)
                                            <option value="{{ $u->id }}"
                                                {{ old('gop_translate_by', $preauth->gop_translate_by) == $u->id ? 'selected' : '' }}>
                                                {{ $u->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                        ...
                        <div class="flex justify-end gap-3 pt-4 border-t border-slate-200 dark:border-slate-700">
                            <button type="button" onclick="toggleEditMode()"
                                class="px-4 py-2 font-medium text-slate-600 hover:text-slate-800 dark:text-slate-400 dark:hover:text-slate-200">Cancel</button>
                            <button type="submit"
                                class="inline-flex items-center rounded-lg bg-emerald-600 px-6 py-2 font-medium text-white shadow-sm hover:bg-emerald-700">Save
                                changes</button>
                        </div>
                    </form>
                </div>
            ...
            <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-800">
                        ...
                        @foreach ($preauth->attachments as $att)
                            @php
                                ...
                                $viewUrl = $isPublicPath
                                    ? asset($att->path)
                                    : route('preauth.attachments.view', [$preauth, $att]);
                                $downloadUrl = $isPublicPath
                                    ? asset($att->path)
                                    : route('preauth.attachments.download', [$preauth, $att]);
                                $ext = strtolower(pathinfo($att->original_name, PATHINFO_EXTENSION));
                                $isImage = in_array($ext, ['jpg', 'jpeg', 'png', 'gif', 'bmp', 'webp'], true);
                                $isPdf = $ext === 'pdf';
                            @endphp
                            <div
                                class="group flex flex-col overflow-hidden rounded-lg border border-slate-200 bg-slate-50 dark:border-slate-600 dark:bg-slate-900">
                                <button type="button"
                                    class="relative block aspect-[4/3] w-full shrink-0 overflow-hidden bg-slate-200 dark:bg-slate-700 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2"
                                    onclick="viewPreauthFile('{{ e($viewUrl) }}', {{ json_encode($att->original_name) }})">
                                    @if ($isImage)
                                        <img src="{{ $viewUrl }}" alt="{{ $att->original_name }}"
                                            class="h-full w-full object-cover transition group-hover:scale-105">
                                    @elseif($isPdf)
                                        <iframe src="{{ $viewUrl }}#toolbar=0&navpanes=0&scrollbar=0"
                                            class="h-full w-full border-0" title="{{ $att->original_name }}"></iframe>
                                    @else
                                    ...
                                    @endif
                                    <span
                                        class="absolute inset-0 flex items-center justify-center rounded-lg bg-black/0 text-white opacity-0 transition group-hover:bg-black/30 group-hover:opacity-100">
                                ...
                                <div class="flex min-w-0 flex-1 flex-col gap-2 p-2">
                                    ...
                                    <div class="flex flex-wrap items-center gap-1">
                                        ...
                                        <a href="{{ $downloadUrl }}"
                                            class="rounded bg-slate-200 px-2 py-1 text-xs font-medium text-slate-700 hover:bg-slate-300 dark:bg-slate-600 dark:text-slate-200 dark:hover:bg-slate-500"
                                            target="_blank" rel="noopener"
                                            @if ($isPublicPath) download="{{ $att->original_name }}" @endif>
                                            <i class="fa-solid fa-download"></i>
                                        </a>
                                        @if (auth()->user()->isAdmin())
                                            <form action="{{ route('preauth.attachments.destroy', [$preauth, $att]) }}"
                                                method="POST" class="inline delete-attachment-form">
                                                @csrf
                                                <button type="button"
                                                    class="delete-attachment-btn rounded bg-red-100 px-2 py-1 text-xs font-medium text-red-700 hover:bg-red-200 dark:bg-red-900/30 dark:text-red-400 dark:hover:bg-red-900/50">
                ...
                @endif
                @if (auth()->user()->isAdmin())
                    <form action="{{ route('preauth.attachments.store', $preauth) }}" method="POST"
                        enctype="multipart/form-data" class="mt-4">
                        @csrf
                        <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Add attachment
                            (PDF or image)</label>
                        <div id="preauth-gop-drop-zone"
                            ...
                            <input type="file" name="gop_attachments[]" id="preauth_gop_attachments" multiple
                            ...
                            <p class="text-sm font-medium text-slate-600 dark:text-slate-400">Drop files here or click to
                                upload</p>
                            <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">PDF, JPEG, PNG, GIF. Max 10MB per
                                file.</p>
                        </div>
                        <ul id="preauth-gop-file-list" class="mt-3 grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-4"
                            aria-live="polite"></ul>
                        <div class="mt-3">
                            <button type="submit"
                                class="inline-flex items-center rounded-lg bg-emerald-600 px-3 py-1.5 text-sm font-medium text-white hover:bg-emerald-700">Upload</button>
                        </div>
                    </form>
                @endif
            </div>
    ...
    <div class="fixed inset-0 z-50 hidden bg-black/50 backdrop-blur-sm" id="preauthFileModal">
        <div
            ...
            <div
                class="flex items-center justify-between border-b border-slate-200 bg-gradient-to-r from-slate-50 to-slate-100 p-6 shadow-sm dark:border-slate-700 dark:from-slate-800 dark:to-slate-700">
                <div class="flex items-center gap-4">
                    <div
                    ...
                    <div>
                        ...
                        <p class="text-sm text-slate-600 dark:text-slate-400">Attachment</p>
                    </div>
                </div>
                <div class="flex items-center gap-3">
                    ...
                    <button type="button"
                        ...
                        onclick="closePreauthFileModal()">
                        <i class="fa-solid fa-times mr-2"></i>Close
                    </button>
                </div>
            </div>
            <div
                class="flex-1 overflow-hidden bg-gradient-to-br from-slate-50 via-white to-slate-100 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900">
    ...
    <script>
        ...
        function viewPreauthFile(fileUrl, filename) {
            ...
            fileContent.innerHTML =
                '<div class="flex flex-col items-center justify-center h-full text-slate-500 dark:text-slate-400"><i class="fa-solid fa-spinner fa-spin text-3xl mb-4"></i><p class="text-lg">Loading file...</p></div>';
            ...
            if (['pdf'].includes(ext)) {
                fileContent.innerHTML = '<iframe src="' + fileUrl +
                    '" class="w-full h-full border-0 rounded-lg shadow-lg" style="min-height: calc(100vh - 120px);"></iframe>';
            } else if (['jpg', 'jpeg', 'png', 'gif', 'bmp', 'webp'].includes(ext)) {
                fileContent.innerHTML = '<div class="w-full h-full flex items-center justify-center"><img src="' + fileUrl +
                    '" class="max-w-full max-h-full object-contain rounded-lg shadow-lg" alt="' + filename +
                    '" style="max-height: calc(100vh - 120px);"></div>';
            } else {
                fileContent.innerHTML =
                    '<div class="flex flex-col items-center justify-center h-full text-center"><div class="bg-white dark:bg-slate-800 rounded-lg p-8 shadow-lg max-w-md"><i class="fa-solid fa-file text-6xl text-slate-400 mb-6"></i><h3 class="text-xl font-semibold text-slate-900 dark:text-white mb-2">' +
                    filename +
                    '</h3><p class="text-slate-600 dark:text-slate-300 mb-6">This file type cannot be previewed in the browser.</p><a href="' +
                    fileUrl +
                    '" target="_blank" class="inline-flex items-center px-6 py-3 bg-blue-600 text-white font-medium rounded-lg hover:bg-blue-700 transition-colors"><i class="fa-solid fa-download mr-2"></i>Download File</a></div></div>';
            }
        }
