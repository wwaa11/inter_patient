                ...
                <div class="border-b border-gray-200 px-6 py-4 dark:border-slate-600">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center">
                            ...
                            <div>
                                <h1 class="flex items-center text-2xl font-bold text-gray-900 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-300">
                                    <i class="fas fa-edit mr-3 text-blue-600"></i>
                                    Edit Main Guarantee ( Update all the same Embassy Ref.)
                                </h1>
                ...
                <div class="mb-6 rounded-lg bg-white shadow-sm dark:bg-slate-800">
                    <div class="space-y-6">
                        <div class="border-b border-gray-200 px-6 py-4 dark:border-gray-700">
                            <h2 class="text-lg font-semibold text-gray-900 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-300">Guarantee Information</h2>
                        </div>

                        <div class="grid grid-cols-1 gap-6 px-6 pb-4 md:grid-cols-3">
                            <!-- Embassy Selection -->
                            <div>
                                <label class="mb-2 block text-sm font-semibold text-gray-700 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-300" for="embassy">
                                    <i class="fas fa-flag mr-2 text-blue-500"></i>Embassy *
                                </label>
                                <select class="@error("embassy") border-red-500 @enderror w-full rounded-lg border border-gray-300 px-4 py-3 transition-all focus:border-transparent focus:ring-2 focus:ring-blue-500 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-100" id="embassy" name="embassy" required>
                                    <option class="dark:bg-slate-800 dark:text-slate-300" value="">Select Embassy</option>
                                    @foreach ($embassies as $embassy)
                                        <option class="dark:bg-slate-800 dark:text-slate-300" value="{{ $embassy->name }}" {{ old("embassy", $guarantee->embassy) == $embassy->name ? "selected" : "" }}>{{ $embassy->name }}</option>
                                    @endforeach
                                </select>
                                @error("embassy")
                                    <p class="mt-1 text-sm text-red-500 dark:text-slate-300">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <input class="@error("embassy_ref") border-red-500 @enderror w-full rounded-lg border border-gray-300 px-4 py-3 transition-all focus:border-transparent focus:ring-2 focus:ring-blue-500 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-100" id="embassy_ref" type="text" name="embassy_ref" required placeholder="Enter embassy reference" value="{{ old("embassy_ref", $guarantee->embassy_ref) }}">
                                @error("embassy_ref")
                            ...
                            <!-- Number -->
                            <div>
                                <label class="mb-2 block text-sm font-semibold text-gray-700 dark:text-slate-300" for="number">
                                    <i class="fas fa-hashtag mr-2 text-green-500"></i>Number (Optional)
                                </label>
                                <input class="@error("number") border-red-500 @enderror w-full rounded-lg border border-gray-300 px-4 py-3 transition-all focus:border-transparent focus:ring-2 focus:ring-blue-500 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-100" id="number" type="text" name="number" placeholder="Enter Number" value="{{ old("number", $guarantee->number) }}">
                                @error("number")
                                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                                @enderror
                            </div>
                ...
                <div class="flex flex-col gap-6 lg:flex-row">
                    ...
                    <div class="rounded-lg bg-white shadow-sm dark:bg-slate-800">
                        <div class="border-b border-gray-200 px-6 py-4 dark:border-slate-700">
                            <h2 class="text-lg font-semibold text-gray-900 dark:text-slate-300">Coverage Area/Specialty</h2>
                        </div>
                        <div class="space-y-6 p-6">
                            ...
                            <!-- Guarantee Cases -->
                            <div>
                                <label class="mb-3 block text-sm font-semibold text-gray-700 dark:text-slate-300">
                                ...
                                <div class="mb-4">
                                    <div class="relative">
                                        <input class="w-full rounded-lg border border-gray-300 px-4 py-2 pl-10 transition-all focus:border-transparent focus:ring-2 focus:ring-blue-500 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-300" id="caseSearch" type="text" placeholder="Search guarantee cases...">
                                ...
                                <!-- Available Cases -->
                                <div class="mb-4">
                                    <h4 class="mb-2 text-sm font-medium text-gray-600 dark:text-slate-300">Available Cases</h4>
                                    <div class="max-h-40 overflow-y-auto rounded-lg border border-gray-200 dark:border-slate-700" id="availableCases">
                                        @foreach ($guaranteeCases as $case)
                                            <div class="case-item flex items-center justify-between border-b border-gray-100 p-3 last:border-b-0 hover:bg-gray-50 dark:border-slate-700 dark:hover:bg-slate-600" data-case-name="{{ strtolower($case->case) }}">
                                                <span class="text-sm text-gray-700 dark:text-slate-300">{{ $case->name }}</span>
                                ...
                                <!-- Selected Cases -->
                                <div>
                                    <h4 class="mb-2 text-sm font-medium text-gray-600 dark:text-slate-300">Selected Cases</h4>
                                    <div class="min-h-16 rounded-lg border-2 border-dashed border-gray-300 bg-gray-50 p-3 dark:border-slate-700 dark:bg-slate-700" id="selectedCases">
                                        @if (count($selectedCases) > 0)
                                            ...
                                            @endforeach
                                        @else
                                            <p class="text-center text-sm text-gray-500 dark:text-slate-300" id="noCasesMessage">No cases selected</p>
                                        @endif
                                    </div>
                                </div>
                            </div>
                                ...
                                @if ($guarantee->file && count($guarantee->file) > 0)
                                    @foreach ($guarantee->file as $file)
                                        ...
                                        @elseif(strtolower($fileExtension) === "pdf")
                                            <embed class="h-full min-h-[700px] w-full rounded-lg p-6" src="{{ asset("hn/" . $patient->hn . "/" . $file) }}" type="application/pdf">
                                        @else
                                            <div class="text-center">
                                                <i class="fas fa-file mb-3 text-4xl text-gray-400 dark:text-slate-300"></i>
                                                <p class="font-medium text-gray-700 dark:text-slate-300">{{ $file }}</p>
                                                <p class="text-sm text-gray-500 dark:text-slate-300">Current file</p>
                                            </div>
                                        @endif
                                    @endforeach
                                @else
                                    <div class="text-center">
                                        ...
                                        <p class="text-gray-500 dark:text-slate-300">No file uploaded</p>
    ...
    <script>
        ...
        function updateSelectedCasesDisplay() {
            ...
            if (selectedGuaranteeCases.length === 0) {
                container.innerHTML = '<p class="text-center text-sm text-gray-500 dark:text-slate-300" id="noCasesMessage">No cases selected</p>';
            } else {
                let html = '';
            ...
            if (file) {
                ...
                if (fileType.startsWith('image/')) {
                    ...
                    preview.innerHTML = `
                        <div class="text-center">
                            <i class="fas fa-file mb-3 text-4xl text-gray-400 dark:text-slate-300"></i>
                            <p class="font-medium text-gray-700 dark:text-slate-300">${fileName}</p>
                            <p class="text-sm text-gray-500 dark:text-slate-300">File uploaded successfully</p>
                        </div>
                        `;
                }
            }
