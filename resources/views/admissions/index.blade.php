                ...
                <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    @if (auth()->user()->isAdmin())
                        <a href="{{ route('admissions.create') }}"
                            class="inline-flex items-center rounded-lg bg-emerald-600 px-4 py-2 font-medium text-white shadow-sm transition-colors duration-200 hover:bg-emerald-700">
            ...
            <form action="{{ route('admissions.index') }}" method="GET"
                ...
                <div class="flex gap-2">
                    ...
                    <a href="{{ route('admissions.index') }}"
                        class="inline-flex items-center rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-medium text-slate-700 shadow-sm hover:bg-slate-50 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-300 dark:hover:bg-slate-600">Reset</a>
                </div>
            </form>
                    ...
                    <table class="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
                        <thead class="bg-slate-50 dark:bg-slate-900">
                            <tr>
                                ...
                                <th
                                    ...
                                    Case Status</th>
                                <th
                                    class="relative whitespace-nowrap px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-slate-500 dark:text-slate-400">
                        ...
                        <tbody class="divide-y divide-slate-200 bg-white dark:divide-slate-700 dark:bg-slate-800">
                            @forelse ($admissions as $adm)
                                @php
                                    $badgeClass = match ($adm->case_status) {
                                        'Close'
                                ...
                                <tr class="{{ $badgeClass }}">
                                    ...
                                    <td class="whitespace-nowrap px-6 py-4 text-sm text-slate-600 dark:text-slate-400">
                                        {{ $adm->admission_date?->format('Y-m-d') ?? '—' }}</td>
                                    <td class="whitespace-nowrap px-6 py-4 text-sm text-slate-600 dark:text-slate-400">
                                        {{ $adm->department ?? '—' }}</td>
                                    <td class="whitespace-nowrap px-6 py-4 text-sm text-slate-600 dark:text-slate-400">
                                        {{ $adm->admitting_status ?? '—' }}</td>
                                    <td class="whitespace-nowrap px-6 py-4">
                                        @php
                                            $statusClass =
                                                $adm->case_status === \App\Models\Admission::CASE_STATUS_CLOSE
                                                    ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900 dark:text-emerald-200'
                                                    : 'bg-amber-100 text-amber-800 dark:bg-amber-900 dark:text-amber-200';
                                        @endphp
                                        <span
                                            class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium {{ $statusClass }}">{{ $adm->case_status ?? '—' }}</span>
                                    </td>
                                    <td class="whitespace-nowrap px-6 py-4 text-sm font-medium flex items-center gap-2">
                                        <a href="{{ route('admissions.show', $adm) }}"
                                            class="inline-flex items-center text-emerald-600 hover:text-emerald-800 dark:text-emerald-400 dark:hover:text-emerald-300">
