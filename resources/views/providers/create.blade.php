        ...
        <div class="mx-auto max-w-2xl px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-8">
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        <h1 class="text-3xl font-bold text-slate-900 dark:text-white">Add Provider</h1>
                        <p class="mt-2 text-slate-600 dark:text-slate-400">Create a new provider record</p>
                    </div>
                </div>
            </div>
            ...
            <div class="rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-800">
                <form action="{{ route('providers.store') }}" method="POST" enctype="multipart/form-data" class="p-8">
                    @csrf
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <textarea name="detail" id="detail" rows="4"
                                class="mt-1 block w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-slate-900 shadow-sm focus:border-emerald-500 focus:outline-none focus:ring-emerald-500 dark:border-slate-600 dark:bg-slate-700 dark:text-white sm:text-sm">{{ old('detail') }}</textarea>
                            @error('detail')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <div class="mt-1 flex items-center space-x-4">
                                <div id="logo-preview-container" class="h-20 w-20 hidden items-center justify-center rounded-lg border border-slate-200 bg-slate-50 dark:border-slate-700 dark:bg-slate-900">
                                    <img id="logo-preview" src="#" alt="Logo Preview" class="h-16 w-16 object-contain">
                                </div>
                                <div id="logo-placeholder" class="h-20 w-20 flex items-center justify-center rounded-lg border-2 border-dashed border-slate-300 bg-slate-50 dark:border-slate-700 dark:bg-slate-900">
    ...
    <script>
        document.getElementById('logo').onchange = evt => {
            ...
            if (file) {
                ...
                document.getElementById('logo-preview-container').classList.add('flex')
